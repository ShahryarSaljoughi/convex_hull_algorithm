...
point get_top(vector<point> &points){
...
void remove_inside_points(list<point> points, const point &l,const point &r){
...
void find_next_vertices(const list<point> points, point &l, point &r){
	...
	for(point p: points){
		if ((p!=l) and (p.y>=l.y) and (polar_angle(l, p)<M_PI)and (polar_angle(l, p)>polar_angle(l, next_l))) next_l = p;
...
vector<point> my_algo_v3(vector<point> &input){
